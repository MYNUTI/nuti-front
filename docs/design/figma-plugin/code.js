...
async function run(opts) {
  ...
  // ★ 최상위로 승격 — 피그마는 최상위 프레임끼리만 프로토타입 연결이 된다
  if (opts.promote) {
    var containers = [];
    found.forEach(function (k) {
      var n = map[k];
      while (n.parent && n.parent.type !== 'PAGE') n = n.parent;
      if (n !== map[k] && containers.indexOf(n) === -1) containers.push(n);
    });
    var origin = { x: 0, y: 0 };
    if (containers.length && containers[0].absoluteBoundingBox) {
      origin.x = containers[0].absoluteBoundingBox.x;
      origin.y = containers[0].absoluteBoundingBox.y;
    }
    // 플로우 순서대로 배치 (열 470px · 행 1020px)
    var COLW = 470, ROWH = 1020, placed = 0, fallbackRow = 9, fallbackCol = 0;
    DATA.screens.forEach(function (s) {
      var f = map[s.key];
      if (!f) return;
      var pos = DATA.layout[s.key];
      if (!pos) { pos = [fallbackCol++, fallbackRow]; }
      page.appendChild(f);
      f.x = origin.x + pos[0] * COLW;
      f.y = origin.y + pos[1] * ROWH;
      placed++;
    });
    // 섹션 제목
    var labeled = 0;
    try {
      ...
      for (var si = 0; si < DATA.sections.length; si++) {
        ...
        var tx = figma.createText();
        tx.fontName = { family: 'Inter', style: 'Bold' };
        tx.characters = sec[1];
        tx.fontSize = 34;
        tx.fills = [{ type: 'SOLID', color: { r: 0.18, g: 0.165, b: 0.141 } }];
        tx.name = 'SECTION ' + sec[1];
        page.appendChild(tx);
        tx.x = origin.x;
        tx.y = origin.y + sec[0] * ROWH - 66;
        labeled++;
      }
    } catch (e) {}
    var removed = 0;
    containers.forEach(function (c) { try { c.remove(); removed++; } catch (e) {} });
    log.push('플로우 배치 완료 — 프레임 ' + placed + '개, 섹션 제목 ' + labeled + '개' + (removed ? ' (임포트 컨테이너 정리)' : ''));
  }
